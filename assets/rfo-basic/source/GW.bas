...
%      GW_ORIENTATION$="landscape|portrait|reverse-landscape|reverse-portrait"
%
...
%      mytxt$+=GW_LINK$ ("http://mylink.com")
%
% TODO TODO TODO TODO TODO TODO TODO TODO TODO TODO TODO TODO TODO TODO TODO TODO TODO TODO TODO TODO TODO TODO TODO
% * (nice-to-have) adapt FLIPSWITCH width to its content
% TODO TODO TODO TODO TODO TODO TODO TODO TODO TODO TODO TODO TODO TODO TODO TODO TODO TODO TODO TODO TODO TODO TODO
%
% Changelog:
% [v5.3] 06-DEC-2020
%      * /!\ Consecutive spaces in user strings are now preserved in all controls
%      * /!\ Action messages received by GW_ACTION$() and GW_WAIT_ACTION$() are now URL-decoded
%            This allows to use messages with spaces and exotic characters in them (although it's not recommended)
%      * /!\ Links created with GW_ADD_LINK() now sit on one row
%      * /!\ GW_DUMP() now copies the page content to the clipboard as well as printing it on the console
%      * Added GW_MODIFY() of a PANEL "content"
%      * Added new CUSTO parameter 'align' for text-alignment, with values 'left', 'center', 'right' and 'justify'
%      * Added GW_ORIENTATION$ variable to lock the screen orientation before loading the GW lib
%        Possible values are: "landscape", "portrait", "reverse-landscape", and "reverse-portrait"
%        (default value is empty string = variable by sensors)
%      * Fixed missing elements in TABLE when the array dimension is not a multiple of the number of columns
%      * Fixed TITLEBAR/FOOTBAR not showing when created with only L/RBUTTON or L/RMENU but no TITLE
%      * Fixed notext icon-only buttons in TITLEBAR/FOOTBAR showing rectangular instead of round
%      * Fixed first space after new line not showing in a multi-line TEXTBOX
%      * Fixed GW_SHOW_DIALOG() not showing dialog when a previous dialog closed shortly before
%      * Fixed GW_ADD_INPUTBOX() of a multi-line text (containing "\n")
%      * Fixed correct height of an INPUTBOX with a custo "rows=n" (n integer)
%      * Fixed GW demo to download 3gp video from laughton.com via HTTP rather than FTP
%      * Removed deprecated SUBMIT controls from the GW demo and used more flexible BUTTON controls instead
% [v5.2] 19-AUG-2019
...
BUNDLE.CREATE gwbundle % name is insignificant, we need at least one bundle

IF GW_ORIENTATION$="landscape"
  BUNDLE.PUT 1, "gw-init-orient", 0
ELSEIF GW_ORIENTATION$="portrait"
  BUNDLE.PUT 1, "gw-init-orient", 1
ELSEIF GW_ORIENTATION$="reverse-landscape"
  BUNDLE.PUT 1, "gw-init-orient", 2
ELSEIF GW_ORIENTATION$="reverse-portrait"
  BUNDLE.PUT 1, "gw-init-orient", 3
ELSE % default: variable by sensors
  BUNDLE.PUT 1, "gw-init-orient", -1
ENDIF

FN.DEF GW_ORIENTGET() % (internal) get screen orientation defined with GW_ORIENTATION$
  BUNDLE.GET 1, "gw-init-orient", o
  FN.RTN o
FN.END % FN.END not followed by \n --> do no count this function in GW_NFN

HTML.OPEN 0, GW_ORIENTGET() % hide status bar, else JQM header is masked (bug)
...
BUNDLE.PUT 1, "gw-last-displayed-dialog", 0
...
FN.DEF GW_CHECK_AND_DOWNLOAD(f$) % (internal) Download a single file to the device (except if it already exists)
  ...
  IF fid<0 THEN BYTE.OPEN r, fid, "http://laughton.com/basic/programs/html/GW%20(GUI-Web%20lib)/GW/" + f$ % fallback
...
FN.DEF GW_DOWNLOAD_THIRD_PARTY(f$) % Download 3rd party lib from laughton.com (fallback: mougino.free.fr) to source/
  ...
  IF fe THEN FN.RTN 1 % already on device!
  BYTE.OPEN r, fid, "http://mougino.free.fr/tmp/GW/third-party-libs/" + f$
  IF fid<0 THEN BYTE.OPEN r, fid, "http://laughton.com/basic/programs/html/GW%20(GUI-Web%20lib)/third-party-libs/" + f$
...
FN.DEF GW_SHOW_DIALOG(ctl_id) % manually trigger a DIALOG (MESSAGE/INPUT/CHECKBOX)
  % Check that any previous dialog is fully closed
  GW_REGISTER("GW_SHOW_DIALOG")
  BUNDLE.GET 1, "gw-last-displayed-dialog", last
  IF last
    last$=GW_ID$(last)
    script$="if(!$('#"+last$+"').parent()"
    script$+=".hasClass('ui-popup-hidden')){"
    script$+="$('#"+last$+"').on('popupafterclose',"
    script$+="function(){RFO('"+last$+"_closed');})}"
    script$+="else{RFO('"+last$+"_closed');}"
    JS(script$)
    GW_WAIT_ACTION$() % "popup_closed"
    JS("$('#"+last$+"').off('popupafterclose')")
  ENDIF
  % Get Unique IDentifier of the control
  ctl$=GW_ID$(ctl_id)
  ...
  ENDIF
  BUNDLE.PUT 1, "gw-last-displayed-dialog", ctl_id
FN.END
IF !GW_SILENT_LOAD THEN GW_LOAD_PG(++GW_CFN, GW_NFN)
...
FN.DEF GW_CLOSE_DIALOG(ctl_id) % manually close a DIALOG (MESSAGE/INPUT/CHECKBOX)
  ...
  ctl$=GW_ID$(ctl_id)
  script$="$('#"+ctl$+"')"
  script$+=".popup('close',{transition:"
  script$+="effect"+ctl$+"})"
  script$+=".on('popupafterclose',"
  script$+="function(){RFO('"+ctl$+"_closed');})"
  JS(script$)
  GW_WAIT_ACTION$() % "popup_closed"
  JS("$('#"+ctl$+"').off('popupafterclose')")
FN.END
IF !GW_SILENT_LOAD THEN GW_LOAD_PG(++GW_CFN, GW_NFN)
...
FN.DEF GW_ADD_DIALOG_INPUT$(title$, msg$, inpu$, btn_axn$[])
  ...
  e$+="<div id='"+u$+"-title'>"+GW_NEW_DLG_TITLE$(title$)+"</div>"
  e$+="<div role='main' class='ui-content'><h3 id='"+u$+"-text' class='ui-title'>"+WEB$(msg$)+"</h3>"
  e$+="<input id='"+u$+"-input' value="+CHR$(34)+WEB$(inpu$)+CHR$(34)+" type='"+typ$+"'>"
...
FN.DEF GW_ADD_BAR_BTN$(ba$, type$) % (internal) for TITLEBAR and FOOTBAR (type$="left"|"right")
  ...
  IF theme$="ios" | theme$="android-holo" | theme$="metro"
    e$="<a "+cs$+" "
  ELSE
    e$="<a class='ui-btn-"+type$+" ui-btn ui-shadow ui-btn-corner-all "+cs$+"' "
  ENDIF
...
FN.DEF GW_ADD_TITLEBAR$(content$)
  ...
  ENDIF
  IF !IS_IN("<h1",e$) THEN e$+="<h1></h1>" % (5.3)
  e$+="</div>"
  % Add to the list of controls + return its content
  GW_ADD_SKEY("control", u$)
  FN.RTN e$
...
FN.DEF GW_THEME_CUSTO$(ctl$) % (internal) Return a formated theme customization for this control family
  ...
  ELSEIF ctl$="group" % group of checkboxes/radios
  ...
  ELSE % default: all other controls
    ...
    i=IS_IN("align=", r$)
    IF i % text-alignment (5.3)
      j=IS_IN(" ", r$, i+1)
      IF !j THEN j=LEN(r$)+1
      a$=MID$(r$,i,j+1-i)
      j=IS_IN("=", a$, i+1)
      GW_REPLACE_IN_STYLE(&r$, a$, "text-align:"+MID$(a$,j+1))
    ENDIF
    i=IS_IN("alpha=", r$)
...
FN.DEF GW_ADD_SPINNER$(msg$)
  ...
  % Create the control
  u$=GW_NEWID$("spinner")
  e$="var tx"+u$+"="+CHR$(34)+WEB$(msg$)+CHR$(34)+";"
...
FN.DEF GW_ICON$(ctl_id)
  GW_REGISTER("GW_ICON$")
  FN.RTN " ui-btn-icon-notext ui-nodisc-icon ui-icon-"+GW_ID$(ctl_id)
FN.END
IF !GW_SILENT_LOAD THEN GW_LOAD_PG(++GW_CFN, GW_NFN)
...
FN.DEF GW_ADD_LINK$(label$, link$)
  ...
  % Create the control
  u$=GW_NEWID$("link")
  e$="<p><a id='"+u$+"' href='"+href$+"' "+GW_THEME_CUSTO$("link")
  e$+=">"+WEB$(label$)+"</a></p>"
  % Add the control to the list of controls + return its content
  GW_ADD_SKEY("control", u$)
  FN.RTN e$
...
FN.DEF GW_ADD_INPUTBOX$(label$, s_ini$)
  % Create the control
  u$=GW_NEWID$("inpbox")
  custo$=GW_THEME_CUSTO$("inpbox")
  e$+="<label for='"+u$+"'>"+WEB$(label$)+"</label>"
  e$+="<textarea name='"+u$+"' id='"+u$+"' "+custo$
  IF !IS_IN("rows=",custo$) THEN e$+="rows='3' "
  e$+=" cols='20'>"+s_ini$+"</textarea>"
  % Add the css, if needed
  IF IS_IN("rows=",custo$) THEN e$+="<style>#"+u$+"{height:100% !important}</style>"
  % Add the control to the list of controls + return its content
  GW_ADD_SKEY("control", u$)
  FN.RTN e$
FN.END
...
FN.DEF GW_RENDER(page)
  ...
  % Rendering a page discards any open dialog
  BUNDLE.PUT 1, "gw-last-displayed-dialog", 0
...
FN.DEF GW_TRANSFORM_DATALINK$(data$) % (internal) used by GW_ACTION$/GW_WAIT_ACTION$
  ...
  ENDIF
  % (5.3) Uncompress double-spaces          % Ascii 32 or 0x20 is normal space " "
  data$=REPLACE$(data$, CHR$(32,160), "  ") % Unicode 160 or 0xa0 is &nbsp; (non-breaking space)
  % (5.3) URL-decode the string
  i=IS_IN("%", data$)
  WHILE i
    IF i<=LEN(data$)-2 THEN data$=LEFT$(data$,i-1)+CHR$(HEX(MID$(data$,i+1,2)))+MID$(data$,i+3)
    i=IS_IN("%", data$, i+1)
  REPEAT
...
FN.DEF GW_MODIFY(ctl_id, key$, txt$)
  ...
  ELSEIF key$="text" & IS_IN("spinner", ctyp$)=1 % SPINNER
    JS("tx"+ctl$+"="+DQ$+WEB$(txt$)+DQ$+";show"+ctl$+"()")
  ...
  ELSEIF (key$="title" | key$="text" | key$="input" | key$="checked") & IS_IN("dlg", ctyp$)=1 % DIALOGs (MESSAGE/INPUT/CHECKBOX)
    IF key$="input"
      ...
      JS("$("+DQ$+"#"+ctl$+"-"+key$+DQ$+").val("+DQ$+WEB$(txt$)+DQ$+")")
    ELSEIF key$="checked"
  ...
  ELSEIF key$="content" & ctyp$ = "panel" % PANELs
    IF LEFT$(txt$,1)<>"<" THEN txt$="<p>"+txt$+"</p>"
    JS("populate("+DQ$+ctl$+DQ$+","+DQ$+WEB$(txt$)+DQ$+")")
  ELSE % Unknown key -> assume HTML attribute
    IF LEFT$(ctl$,1)<>"." THEN ctl$="#"+ctl$ % handle MODIFYing a whole class=multiple controls at once
...
IF !IS_APK()
  ...
  gw$="http://mougino.free.fr/tmp/GW/GW.bas": BYTE.OPEN r, gwi, gw$
